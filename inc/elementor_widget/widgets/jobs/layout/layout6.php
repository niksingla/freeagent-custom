...
<div class="jws_job_wap">
    ...
    <div class="top">
        <div class="jws_job_image">
            ...
            <div class="content_heading">
                <p class="name_emp"><span class="name"><?php echo ''.$display_name?></span> <?php echo ''.$verified_lable;?></p>
                <h6 class="entry-title"><a href="<?php echo get_permalink(); ?>"><?php  echo ''.(!empty($settings['title_length'])) ? wp_trim_words( $title, $settings['title_length'], $settings['title_more'] ) : $title;?></a></h6>  
            </div>
        </div>
     <div class="jws_job_content">
         <div class="job_infor">
             <?php
                 $term_list = get_the_term_list($post_id, 'jobs_cat', '', ', ', '');
            
                // Check if terms were found
                if ($term_list && !is_wp_error($term_list)) {
                    // Echo the term list
                    echo '<div class="job_cat">'.$term_list.'</div>';
                }
             ?>
            <?php
    ...
    <div class="btn_actions">
        <a href="<?php the_permalink(); ?>" class="btn_view"><?php echo esc_html__('View Detail','freeagent');?><i class="jws-icon-long-arrow"></i></a>
    </div>
</div>