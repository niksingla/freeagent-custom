...
<div class="jws_job_wap">
 ...
 <h6 class="entry-title"><a href="<?php echo get_permalink(); ?>"><?php  echo ''.(!empty($settings['title_length'])) ? wp_trim_words( get_the_title(), $settings['title_length'], $settings['title_more'] ) : get_the_title();?>
        <?php
    if($featured==1){
 ...
 <div class="jws_job_content">
     <div class="job_infor">
         <div class="post_cat">
         <?php 
         $terms = get_the_terms($post_id, 'jobs_cat');
            
            if ($terms && !is_wp_error($terms)) {
                $first_term = reset($terms); // Get the first term
            
                echo '<div class="program_categories">';
                echo '<a href="' . esc_url(get_term_link($first_term)) . '" rel="tag">' . esc_html($first_term->name) . '</a>';
                echo '</div>';
            }
         ?>
         </div>
        ...
        ?></div>
        <div class="total_proposals"><?php echo esc_html__('Proposals ','freeagent');?><span class="number_proposals"><?php echo ''.$proposal_count;?></span></div>
     </div>
