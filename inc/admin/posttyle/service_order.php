...
if ( ! defined( 'ABSPATH' ) ) {
    ...
    function jws_register_service_order() {
  
		$labels = array(
			'name'                => _x( 'Services Order', 'Post Type General Name', 'freeagent' ),
			'singular_name'       => _x( 'Services Order', 'Post Type Singular Name', 'freeagent' ),
			'menu_name'           => esc_html__( 'Services Order', 'freeagent' ),
			'parent_item_colon'   => esc_html__( 'Parent Item:', 'freeagent' ),
			'all_items'           => esc_html__( 'Services Order', 'freeagent' ),
        ...
        if(function_exists('custom_reg_post_type')){
        	custom_reg_post_type( 'service_order', $args );
        }


	};
add_action( 'init', 'jws_register_service_order', 1 );

function custom_service_order_featured_columns($columns) {
   
    $columns['author'] =  __('Author', 'freeagent');
    
    return $columns;
}
add_filter('manage_service_order_posts_columns', 'custom_service_order_featured_columns');