...
if ( ! defined( 'ABSPATH' ) ) {
    ...
    function jws_register_proposal() {
  
		$labels = array(
			'name'                => _x( 'Proposal', 'Post Type General Name', 'freeagent' ),
			'singular_name'       => _x( 'Proposal', 'Post Type Singular Name', 'freeagent' ),
			'menu_name'           => esc_html__( 'Proposal', 'freeagent' ),
			'parent_item_colon'   => esc_html__( 'Parent Item:', 'freeagent' ),
			'all_items'           => esc_html__( 'Proposal', 'freeagent' ),
        ...
        if(function_exists('custom_reg_post_type')){
        	custom_reg_post_type( 'job_proposal', $args );
        }


	};
add_action( 'init', 'jws_register_proposal', 1 );
