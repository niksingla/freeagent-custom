...
function particles_background_script( $scope ) {
	...
	particles_style = $scope.find( '.jws-particle-wrapper' ).data( 'jws-partstyle' );

	if( typeof particles_style != 'undefined' ) {
		if( element_type == 'column' ) {
			$column = $scope.find( '.elementor-column-wrap .elementor-background-overlay' );
			$particle_wrap = $scope.find( '.jws-particle-wrapper' );
			if( $column.next( '#jws-particle-' + id ).length == 0 ) {
				$column.after( $particle_wrap );
			}
		} else {
			$bg_wrap = $scope.find( '.elementor-element-overlay ~ .elementor-background-overlay' );
			$particle_wrap = $scope.find( '#jws-particle-' + id );
			if( $bg_wrap.next( '#jws-particle-' + id ).length == 0 ) {
				$bg_wrap.after( $particle_wrap );
			}
		}
		$selector = $scope.find( '#jws-particle-' + id );

	} else {

		particles_style = $scope.data( 'jws-partstyle' );

		$selector = $scope;
		$content = $( '<div class="jws-particle-wrapper" id="jws-particle-' + id + '"></div>' );
	...
	var particles_dot_color = $selector.data( 'jws-partcolor' ),
		particles_custom = $selector.data( 'jws-partdata' ),
		particles_opacity = $selector.data( 'jws-partopacity' ),
		particles_direction = $selector.data( 'jws-partdirection' );
        var particles_dot = [particles_dot_color];
	if( particles_custom !== '' && particles_style == 'custom' ) {
		particlesJS( 'jws-particle-' + id, particles_custom );
		...
		if ( $scope.hasClass( 'jws-particle-adv-yes' ) ) {
			var particles_number = $selector.data( 'jws-partnum' );
			var particles_size = $selector.data( 'jws-partsize' );
			var particles_speed = $selector.data( 'jws-partspeed' );
			var particles_interactive = $selector.data( 'jws-interactive' );
		...
		particlesJS( 'jws-particle-' + id, config );
	}
}
