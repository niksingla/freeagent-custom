...
(function($) {
    ...
    if($('.jws_jobs_move').length) {
        $('.jws_jobs_move').stick_in_parent({offset_top: 0}).on("sticky_kit:bottom", function(e) {
            $('.jws_jobs_move').addClass('is_bottom')
        }).on("sticky_kit:unbottom", function(e) {
            $('.jws_jobs_move').removeClass('is_bottom')
        }); 
    } 
}(jQuery));